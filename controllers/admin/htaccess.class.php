...
/**
 * .htaccess Editor Admin Controller
 *
...
class AdminHtaccess extends ModuleAdminController implements Module_Admin_Controller_Interface
{
    ...
    protected function setup()
    {
        ...
        add_filter('plugin_action_links_' . $this->core->modules('htaccess')->basename(), array($this, 'settings_link'));
    ...
    public function admin_menu()
    {
        ...
        if (defined('O10N_WPO_VERSION') || count($this->core->modules()) > 1) {
            add_submenu_page('o10n', __('.htaccess Editor', 'o10n'), __('.htaccess', 'o10n'), 'manage_options', 'o10n-htaccess', array(
                 &$this->AdminView,
                 'display'
             ));
        ...
        // add menu entry
            add_submenu_page('tools.php', __('.htaccess Editor', 'o10n'), __('.htaccess Editor', 'o10n'), 'manage_options', 'o10n-htaccess', array(
                 &$this->AdminView,
                 'display'
             ));
        }
    ...
    {
        $settings_link = '<a href="'.esc_url(add_query_arg(array('page' => 'o10n-htaccess','tab' => 'editor'), admin_url($this->admin_base))).'">'.__('Editor').'</a>';
    ...
    {
        if ($file == $this->core->modules('htaccess')->basename()) {
    ...
    {
        ?><script>jQuery(function($){var r=$('*[data-plugin="<?php print $this->core->modules('htaccess')->basename(); ?>"]');
            $('.plugin-title strong',r).html('<?php print $this->core->modules('htaccess')->name(); ?><a href="https://optimization.team" class="g100" style="font-size: 10px;float: right;font-weight: normal;opacity: .2;line-height: 14px;">O10N</span>');
